...
class OptimizerChainFactory
{
     ...
     */
    public static function create(array $config = []): \Spatie\ImageOptimizer\OptimizerChain
    {
        $defaultQuality = 85;
        ...
        if ($isRemote) {
            ...
            $optimizerChain->addOptimizer(new \Swissup\Pagespeed\Image\Optimizers\Remote($options));
        } else {
            ...
            if (isset($config['convert_to_webp']) && $config['convert_to_webp'] === true) {
                $optimizerChain
                    ->addOptimizer(new \Swissup\Pagespeed\Image\Optimizers\ConvertToWebp([
                        '-m 6',
                        '-pass 10',
                        '-mt',
                        '-q ' . $quality,
