...
class Remote extends \Spatie\ImageOptimizer\Optimizers\BaseOptimizer
{
    ...
    {
        $url = isset($this->options['baseUrl']) ? $this->options['baseUrl']: '';
    ...
    {
        return (string) $this->options['apiUrl'];
    }
    ...
    {
        return (string) $this->options['apiKey'];
    }
    ...
    {
        $mediaDir = (string) $this->options['mediaDir'];
        $mediaDir = rtrim($mediaDir, '/');
        $filename = (string) $this->imagePath;
...
//                'timeout'  => 2.0,
    ...
    private function optimize()
    {
        ...
        $response = $client->request('POST', "{$this->getApiUrl()}/image/optimize", [
            ...
            'multipart' => [
                ...
                [
                    'name'     => 'image',
                    'contents' => \GuzzleHttp\Psr7\Utils::tryFopen($this->imagePath, 'r')
                ]
            ]
        ]);
    ...
    public function getCommand(): string
    {
        ...
        if ($result
            ...
            $this->download($result['dest'], $this->imagePath);
        }
