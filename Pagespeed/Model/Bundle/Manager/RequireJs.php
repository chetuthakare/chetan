...
namespace Swissup\Pagespeed\Model\Bundle\Manager;
...
class RequireJs implements BundleInterface
{
     ...
     * @var string
     */
    private $handle = 'Swissup_Pagespeed/js/bundle/default';
    ...
    public function __construct(
        ...
        $this->staticDir = $directoryWrite;
        $this->area = $area;
        $this->theme = $theme;
        $this->locale = $locale;
        $this->contentPools = array_merge($this->contentPools, $contentPools);
        $this->pathToBundleDir = $this->area . '/' . $this->theme . '/' . $this->locale . '/';
    }
    ...
    public function setHandle($handle)
    {
        ...
        $replacements = [
            'bundles/shared'                => 'Swissup_Pagespeed/js/bundle/default',
            'bundles/cms-index-index'       => 'Swissup_Pagespeed/js/bundle/cms-index-index',
            'bundles/catalog-category-view' => 'Swissup_Pagespeed/js/bundle/catalog-category-view',
            'bundles/catalog-product-view'  => 'Swissup_Pagespeed/js/bundle/catalog-product-view',
            'bundles/checkout-index-index'  => 'Swissup_Pagespeed/js/bundle/checkout-index-index',
            'bundles/checkout-cart-index'   => 'Swissup_Pagespeed/js/bundle/checkout-cart-index',
        ];

        $handle = isset($replacements[$handle]) ? $replacements[$handle] : $handle;
     ...
     * @return boolean
     */
    public function isHandleAlreadyExist()
    {
        $filename = $this->handle . '-' . 'bundle' . 0 . '.js';
        return $this->staticDir->isExist(
            $this->minification->addMinifiedSign($this->pathToBundleDir . '/' . $filename)
        );
    }
    ...
    {
        $filename = $this->handle . '-' . 'bundle' . $this->bundleFileIndex . '.js';
    ...
    private function getFileContent($sourcePath)
    {
        if (!isset($this->fileContent[$sourcePath])) {
            if ($this->staticDir->isExist(
                $this->minification->addMinifiedSign($sourcePath)
            )) {
                $content = $this->staticDir->readFile(
                    $this->minification->addMinifiedSign($sourcePath)
                );
            } else {
            ...
            $content = preg_replace(
                '/\/\/(#|@)\s(sourceURL|sourceMappingURL)=\s*(\S*?)\s*$/m',
                '',
                $content
            );
